<template>
    <div class="search-container sticky-top md-background mb-0">
        ...
        <div class="search-input">
            <input ref="search_field" type="text" class="body-large" :placeholder="t('search_reddit')" @input="search"
                @keyup.enter="search">
        </div>
...
async function get_results() {
    ...
    let response = await redly.searchAll(search_field.value.value);
    if (!response) {
...
async function scroll() {
    ...
    if (section.value == 'all') {
        response = await redly.searchAll(search_field.value.value, null, {
            after: after.value
        });
    }

    else if (section.value == 'posts') {
        response = await redly.searchSubmissions(search_field.value.value, {
            after: after.value
        });
    }

    else if (section.value == 'users') {
        response = await redly.searchUsers(search_field.value.value, {
            after: after.value
        });
    }

    else if (section.value == 'communities') {
        response = await redly.searchSubreddits(search_field.value.value, {
            after: after.value
        });
    }
...
async function search_posts() {
    ...
    let response = await redly.searchSubmissions(search_field.value.value);
    if (!response) {
        scroll_loaded.value = true;
        return;
    }
...
async function search_users() {
    ...
    let response = await redly.searchUsers(search_field.value.value);
    if (!response) {
        scroll_loaded.value = true;
        return;
    }
...
async function search_communities() {
    ...
    let response = await redly.searchSubreddits(search_field.value.value);
    if (!response) {
        scroll_loaded.value = true;
        return;
    }
