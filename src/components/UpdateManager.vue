...
async function update() {
    ...
    await AppLauncher.openUrl({
        url: `https://github.com/dinler03/redly/releases/download/${tag}/Redly_${tag}.apk`
    })
}
...
async function check_for_updates() {
    let response = await fetch("https://api.github.com/repos/dinler03/redly/releases/latest")
        .then(res => res.json())
        .catch(err => null);
